...
public interface SlotRepository extends CrudRepository<Slot, SlotKey>{
    ...
    @Transactional
    @Modifying
    @Query("delete from Slot s where s.id.permissoes.id.cartao.id = :idCartao and s.id.permissoes.id.fechadura.id = :idFechadura")
    void deleteAllByPermissoesIdCartaoAndPermissoesIdFechadura(@Param("idCartao") String idCartao, @Param("idFechadura") Integer idFechadura);
